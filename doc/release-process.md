...
### TechsNetworkcoin maintainers/release engineers, suggestion for writing release notes
...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in techsnetworkcoin, then:
...
### Build and sign TechsNetworkcoin Core for Linux, Windows, and macOS:

    export GITIAN_THREADS=2
    export GITIAN_MEMORY=3000
    
    pushd ./gitian-builder
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit techsnetworkcoin=v${VERSION} ../techsnetworkcoin/contrib/gitian-descriptors/gitian-linux.yml
    ...
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit techsnetworkcoin=v${VERSION} ../techsnetworkcoin/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit techsnetworkcoin=v${VERSION} ../techsnetworkcoin/contrib/gitian-descriptors/gitian-osx.yml
...
Build output expected:
  ...
  4. macOS unsigned installer and dist tarball (`techsnetworkcoin-${VERSION}-osx-unsigned.dmg`, `techsnetworkcoin-${VERSION}-osx64.tar.gz`)
  5. Gitian signatures (in `gitian.sigs.ltc/${VERSION}-<linux|{win,osx}-unsigned>/(your Gitian key)/`)

### Verify other gitian builders signatures to your own. (Optional)

Add other gitian builders keys to your gpg keyring, and/or refresh keys: See `../techsnetworkcoin/contrib/gitian-keys/README.md`.
...
Codesigner only: Sign the macOS binary:
    ...
    transfer techsnetworkcoin-osx-unsigned.tar.gz to macOS for signing
    tar xf techsnetworkcoin-osx-unsigned.tar.gz
...
Codesigner only: Sign the windows binaries:
    ...
    tar xf techsnetworkcoin-win-unsigned.tar.gz
...
Codesigner only: Commit the detached codesign payloads:
    ...
    cd ~/techsnetworkcoin-detached-sigs
...
- Announce the release:
  ...
  - techsnetworkcoin-dev mailing list
