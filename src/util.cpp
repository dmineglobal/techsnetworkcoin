...
const char * const BITCOIN_CONF_FILENAME = "techsnetworkcoin.conf";
const char * const BITCOIN_PID_FILENAME = "techsnetworkcoin.pid";
...
#else
    const char* pszModule = "techsnetworkcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Techsnetworkcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Techsnetworkcoin";
#else
    // Unix
    return pathRet / ".techsnetworkcoin";
#endif
#endif
}
